var chaos = function () {
    ...
    var DataPoint = function (index, previousValue, value) {
        this.index = index;
        this.previousValue = previousValue;
        this.value = value;
    };

    var updateKLabel = function (labelId, newValue) {
    ...
    return {
        ...
        updateLogisticSlider: function (slideAmount) {
            updateKLabel("logisticKLabel", slideAmount);
            $("#logisticGraph1").remove();
            $("#logisticGraph2").remove();
            this.graph("logisticGraph", this.logistic, 0.4, slideAmount);
        },

        updateQuadraticSlider: function (slideAmount) {
            updateKLabel("quadraticKLabel", slideAmount);
            $("#quadraticGraph1").remove();
            $("#quadraticGraph2").remove();
            this.graph("quadraticGraph", this.quadratic, 0.4, slideAmount);
        },

        graph: function (graphName, f, x, k) {
            ...
            xScale1.domain(d3.extent(data, function (d) {
                return d.index;
            }));
            yScale1.domain(d3.extent(data, function (d) {
                return d.value;
            }));
